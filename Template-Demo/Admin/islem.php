    ...
    if(isset($_POST['kayit_Menu'])) {
        ...
        if(isset($_POST['kayit_Footer'])) {
            ...
            if(isset($_POST['ders_Duzenle'])) {
                $ders_ad_duzen = $_POST['ders_ad_duzenle'];
                $ders_icerik_duzen = $_POST['ders_icerik_duzenle'];
                $ders_link_duzen = $_POST['ders_link_duzenle'];
                $ders_kodlama_link_duzen = $_POST['ders_kodlama_link_duzenle'];
                $dersler_id = $_POST['dersler_id'];
                if($ders_ad_duzen && $ders_link_duzen && $ders_icerik_duzen && $ders_kodlama_link_duzen  ){
            
                    $ders_varmi = $baglan->query("select * from dersler where dersler_id = '$dersler_id'");
                    $ders_say = $ders_varmi->num_rows;
                    
                    
                    if($num_rows == 0){
                        
                        $ders_guncelle = $baglan->query("update dersler  set ders_ad='".$_POST['ders_ad_duzenle']."'
                        ,ders_icerik='".$_POST['ders_icerik_duzenle']."',ders_link='".$_POST['ders_link_duzenle']."',ders_kodlama_link='".$_POST['ders_kodlama_link_duzenle']."'
                        where dersler_id ='$dersler_id'");
                        ...
                        header('Location:menu-Duzenle.php?ders-ekle=denied');
            ...
            if($_GET['derskaldir']=="ok"){

                $derskaldir = $baglan->query("delete from  dersler where dersler_id='".$_GET['ders_id']."'");
            ...
            if(isset($_POST['ders_Kayit_Et'])) {
                $ders_ad = $_POST['ders_ad'];
                $ders_icerik = $_POST['ders_icerik'];
                $ders_sira = $_POST['ders_sira'];
                $ders_sayfa = $_POST['ders_sayfa'];
                $zaman = date('Y-m-d H-i');
                $ders_kodlama_link = $_POST['ders_kodlama_link'];
                $ders_link = $_POST['ders_link'];
                
                if($ders_ad && $ders_icerik && $ders_sira && $ders_sayfa && $ders_kodlama_link && $ders_link  ){
            
                    $ders_varmi = $baglan->query("select * from dersler where ders_ad = '$ders_ad' or ders_link = '$ders_link'");
                    $ders_say = $ders_say->num_rows;
                    
                    
                    if($ders_say==0){
                        $ders_kaydet = $baglan->query("insert into  dersler (ders_ad, ders_icerik,ders_sira,ders_sayfa,ders_link,ders_kodlama_link,ders_tarih) values ('$ders_ad','$ders_icerik','$ders_sira','$ders_sayfa','$ders_kodlama_link','$ders_link','$zaman')");
                        header('Location:sayfalar.php');
                        
                        
                    } else {
                        
                        header('Location:menu-ekle.php?sayfa-ekle=denied');
